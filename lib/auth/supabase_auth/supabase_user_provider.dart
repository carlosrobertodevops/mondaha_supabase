...
class MordahaSupabaseUser extends BaseAuthUser {
  MordahaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> mordahaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MordahaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
